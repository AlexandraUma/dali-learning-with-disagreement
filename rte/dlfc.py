# -*- coding: utf-8 -*-
"""softmax_dlfc_rte_complete.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1xfrS0Z-BXtOoZf3-O0KkA6QI0BI49zML
"""
...
train_distr = []
train_soft = []
filename1 = 'drive/My Drive/Data/rte/train_anno_users.npy'
train_users = np.load(filename1).tolist()
filename2 = 'drive/My Drive/Data/rte/train_anno_status.npy'
train_status =  np.load(filename2).tolist()
hotsize = 2
...
class RTE_model(torch.nn.Module):
    ...
    def forward(self, text, hypothesis, anno_users, anno_status, test_mode=True):
        ...
        if test_mode:
            ...
            cross_entropy = torch.mul(judgements_one_hot, torch.nn.functional.log_softmax(expected_user_responses, 1))
            loss = -torch.sum(cross_entropy)

            return loss, None, None, softmax_scores

"""**Training on DlFC hot**"""
...
for exp in range(NUM_EXPERIMENTS):
    ...
    for i in range(10):
        ...
        train_y1 = train_users[:start] + train_users[end:]
        train_y2 = train_status[:start] + train_status[end:]
        ...
        train_y2, test_y2 = torch.tensor(train_y2).long().cuda(), torch.tensor(test_y2).float().cuda()
        ...
        for epoch in range(1, num_epochs+1):
            ...
            model.train()
            loss, _,_, _ = model(train_x1, train_x2, train_y1, train_y2, False)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

        model.eval()
        _, _,_,test_pred = model(test_x1, test_x2, test_y1, test_y2)
    ...
    dlfc_dictionary[exp] = test_preds_soft
...
with open(writepath+'rte_dlfc.jsonlines', 'w') as f:
    json.dump(dlfc_dictionary, f)
