# -*- coding: utf-8 -*-
"""softmax_rte_ct.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Mj4qKhhelE5jH7O9mbvBvnLV3tpeluC2
"""
...
class RTE_model(torch.nn.Module):
    ...
    def forward(self, text, hypothesis, one_hot_labels, soft_labels):
        ...
        soft_loss = torch.nn.MSELoss(reduction='sum')(soft_labels, softmax_scores)
...
for exp in range(NUM_EXPERIMENTS):
    ...
    for i in range(10):
        ...
        train_y2 = train_srs[:start] + train_srs[end:]
    ...
    ct_dictionary[exp] = test_preds_soft
...
with open(writepath+'rte_ct.jsonlines', 'w') as f:
...
print('\nCT_SRS  entropy similarity stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))
...
print('\nCT_SRS cross entropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
