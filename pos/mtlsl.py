...
class RNN_all(torch.nn.Module):
    ...
    def forward(self, wword_pad, wword_iis, cchar_pad, one_hot_labels, soft_labels, is_eval=True):
        ...
        if not self.exp == 'mtl':
            ...
            if is_eval:
                ...
                return softmax_scores, softmaxed, None, None
            else:
                softmax_scores = torch.nn.functional.softmax(pred_hot, 1) + 1e-4
                softmaxed = torch.nn.functional.softmax(soft_pred, 1) + 1e-4
                cross_entropy = torch.mul(one_hot_labels, softmax_scores.log())
                hard_loss = -torch.sum(cross_entropy)
                soft_loss = torch.sum(torch.mul(softmaxed, (torch.div(softmaxed, soft_labels).log()))) # inverse kl
                return softmax_scores, softmaxed, hard_loss, soft_loss


"""**Training using Fornaciari MTL**"""
...
for exp in range(NUM_EXPERIMENTS):
    ...
    for epoch in range(num_epochs):
        ...
        for word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat in train_loader:
            hard_predictions, soft_predictions, hard_loss, soft_loss = model(word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat, False)
            backprop_both(optimizer, hard_loss, soft_loss)
    ...
    del model
    model = RNN_all('mtl').cuda()
    model.load_state_dict(torch.load(DATA_PATH+'/best_model.pt'))
    test_predictions, test_preds_soft = get_predictions(model, test_loader, True)
    ...
    dirr = str(exp)
    mtlSoft_dictionary[dirr] = [item.tolist() for item in test_preds_soft]
    mtlHard_dictionary[dirr] = [int(i) for i in test_predictions]
...
with open(writepath+'gimpelpos_hard_' + name + '.jsonlines', 'w') as f:
    json.dump(mtlHard_dictionary, f)

with open(writepath+'gimpelpos_soft_' + name + '.jsonlines', 'w') as f:
...
print('\nMTL entropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
...
print('MTL Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(dev_accs)*100, np.max(dev_accs)*100, np.min(dev_accs)*100, np.std(dev_accs)*100))
