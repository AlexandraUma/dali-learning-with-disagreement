...
class RNN_all(torch.nn.Module):
    ...
    def forward(self, wword_pad, wword_iis, cchar_pad, one_hot_labels, soft_labels):
        ...
        softmax_scores = torch.nn.functional.softmax(pred_hot, 1) + 1e-3
        if self.smythe == 'ce':
...
print(np.average(item_weights))

right_indices = item_weights >= 0 #0.724
print(right_indices, np.sum(right_indices))
...
train = data_utils.TensorDataset(word_pad_trn_tens[right_indices], word_iis_trn_tens[right_indices], char_pad_trn_ten[right_indices], hot_trn_tens[right_indices], soft_trn_tens[right_indices])
...
for exp in range(NUM_EXPERIMENTS):
    ...
    filtering_dictionary[str(exp)] = [item.tolist() for item in test_preds_soft]
...
with open(writepath+'gimpelpos_' + name + '.jsonlines', 'w') as f:
    json.dump(filtering_dictionary, f)
...
print(name.upper() + ' Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(dev_accs)*100, np.max(dev_accs)*100, np.min(dev_accs)*100, np.std(dev_accs)*100))

print('\n' + name.upper() + 'Filtering_Gold PRF stats after 30 epochs:... ')
...
for exp in range(NUM_EXPERIMENTS):
    ...
    filteringMV_dictionary[str(exp)] = [item.tolist() for item in test_preds_soft]
...
with open(writepath+'gimpelpos_' + name + '.jsonlines', 'w') as f:
    json.dump(filteringMV_dictionary, f)


print('Filtering_MV Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(accs)*100, np.max(accs)*100, np.min(accs)*100, np.std(accs)*100))

print('\nFiltering_MV PRF stats after 30 epochs:... ')
avgs = ['averages'] + np.average(prfs, 0).tolist()
maxs = ['maximums'] + np.max(prfs, 0).tolist()
mins = ['minimums'] + np.min(prfs, 0).tolist()
stds = ['stds'] + np.std(prfs, 0).tolist()
...
print('\nFiltering_MV Crowdtruth PRF stats after 30 epochs:... ')
avgs = ['averages'] + np.average(ct_prfs, 0).tolist()
maxs = ['maximums'] + np.max(ct_prfs, 0).tolist()
mins = ['minimums'] + np.min(ct_prfs, 0).tolist()
stds = ['stds'] + np.std(ct_prfs, 0).tolist()
...
print('\nFiltering_MV JSD stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(jsds), np.max(jsds), np.min(jsds), np.std(jsds)))

print('\nFiltering_MV KL stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(kls), np.max(kls), np.min(kls), np.std(kls)))

print('\nFiltering_MV entropy similarity stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))

print('\nFiltering_MV entropy correlation stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ents_correlation), np.max(ents_correlation), np.min(ents_correlation), np.std(ents_correlation)))

print('\nFiltering_MV crossentropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))


print('\n\nEVALUATING ON THE DEV SET I.E. THE FORNACIARA TEST SET')
print('Filtering_MV Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(dev_accs)*100, np.max(dev_accs)*100, np.min(dev_accs)*100, np.std(dev_accs)*100))
