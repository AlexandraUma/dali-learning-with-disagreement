...
for item_idx in range(len(train_distr)):
    ...
    for label_idx in range(hotsize):
        ...
        for each_time in range(num_votes):
            dummy[label_idx] = 1
            # print(num_votes, label_idx, each_time, dummy)
            repeated_labels.append(dummy)
            repeated_indices.append(item_idx)
print('repeated labels and indices')
print(len(repeated_indices), len(repeated_labels))
...
train = data_utils.TensorDataset(word_pad_trn_tens[repeated_indices], word_iis_trn_tens[repeated_indices], char_pad_trn_ten[repeated_indices], hot_trn_tens, soft_trn_tens[repeated_indices])
...
for exp in range(NUM_EXPERIMENTS):
    ...
    dirr = str(exp)
    repeated_dictionary[dirr] = [item.tolist() for item in test_preds_soft]
...
print('Repeated_Labelling Accuracy stats after 30 experiments: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(accs)*100, np.max(accs)*100, np.min(accs)*100, np.std(accs)*100))

print('\nRepeated_Labelling PRF stats after 30 epochs:... ')
...
print('\nRepeated_Labelling Crowdtruth PRF stats after 30 experiments:... ')
...
print('\nRepeated_Labelling JSD stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(jsds), np.max(jsds), np.min(jsds), np.std(jsds)))

print('\nRepeated_Labelling KL stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(kls), np.max(kls), np.min(kls), np.std(kls)))

print('\nRepeated_Labelling entropy similarity stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))

print('\nRepeated_Labelling entropy correlation stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ents_correlation), np.max(ents_correlation), np.min(ents_correlation), np.std(ents_correlation)))

print('\nRepeated crossentropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
