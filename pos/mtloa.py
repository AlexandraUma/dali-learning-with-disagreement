# -*- coding: utf-8 -*-
"""softmax_pos_mtl_uma.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rQIvZ0kgri3w0ArOx3RDfdtKJFTQWXqw
"""
...
def get_predictions(model, eval_loader):
    ...
    for wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat in eval_loader:
        one_hot_pred, item_difficulty,_,_ = model(wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat, None, True)
        ...
        items_diffs.extend(item_difficulty.squeeze(1).detach().cpu().numpy())
    return hard_preds, soft_preds, items_diffs
...
test_diffs = np.array([observed_agreement(a) for a in test_distr])
...
class RNN_all(torch.nn.Module):
    def __init__(self):
        ...
        self.output_diff = torch.nn.Linear(out_final, 1)
...
tensorized_weights = torch.tensor(item_weights).float().cuda().unsqueeze(1)
...
for exp in range(NUM_EXPERIMENTS):
    ...
    for epoch in range(num_epochs):
        ...
        for word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat, oa_weights in train_loader:
            _,_ , hard_loss, aux_loss = model(word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat, oa_weights)
            backprop_both(optimizer, hard_loss, aux_loss)
        
        # evaluate after each epoch using 
        dev_hard_preds, dev_soft_preds,_ = get_predictions(model, dev_loader)
    ...
    test_predictions, test_preds_soft, test_pred_diffs = get_predictions(model, test_loader)
    test_labels = np.argmax(hot_dev,1)

    mtl_OA_dictionary[str(exp)] = [item.tolist() for item in test_preds_soft]
...
"""**you haven't done from here Using MV**"""
...
for exp in range(NUM_EXPERIMENTS):
    ...
    print(test_acc, test_f, cf, jsd, kl, ent, corr, itdiff, itcorr)
    print('#'*50)
