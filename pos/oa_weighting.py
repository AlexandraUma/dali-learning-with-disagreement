# -*- coding: utf-8 -*-
"""softmax_weighted_pos.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1_IlCJHq1yVAIv0MS3JPRw8TnjfY4fvUY
"""
...
def get_predictions(model, eval_loader, mtl=False):
    ...
    for wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat in eval_loader:
        if not mtl:
            one_hot_pred, _,_,_ = model(wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat, None, True)
...
train_ds = np.argmax(soft_ds_trn, 1)
hot_trn_ds = np.eye(hotsize)[train_ds]
print(hot_trn_ds.shape)
sum(np.argmax(hot_trn_ds, 1) == train_ds)
...
item_weights = np.array([observed_agreement(a) for a in train_distr])
...
class RNN_all(torch.nn.Module):
    ...
    def forward(self, wword_pad, wword_iis, cchar_pad, one_hot_labels, soft_labels, weights, eval=False):
        ...
        if eval:
            weights = torch.ones(len(one_hot_labels)).cuda()
        if self.exp == 'stl':
...
tensorized_weights = torch.tensor(item_weights).float().cuda()

train = data_utils.TensorDataset(word_pad_trn_tens, word_iis_trn_tens, char_pad_trn_ten, hot_trn_tens, soft_trn_tens, tensorized_weights)
...
for exp in range(NUM_EXPERIMENTS):
    ...
    for epoch in range(num_epochs):
        ...
        for word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat, oa_weights in train_loader:
            hard_predictions, _, hard_loss, _ = model(word_pad_trn_bat, word_iis_trn_bat, char_pad_trn_bat, y_hot_trn_bat, y_soft_trn_bat, oa_weights)
        ...
        if dev_f > best_val_f:
            ...
            best_val_r = dev_r
            best_val_p = dev_p
            torch.save(model.state_dict(), DATA_PATH+'/best_weighted_model.pt')
    ...
    model.load_state_dict(torch.load(DATA_PATH+'/best_weighted_model.pt'))
...
print('OA_MV Accuracy stats after 30 experiments: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(accs)*100, np.max(accs)*100, np.min(accs)*100, np.std(accs)*100))
...
print('\nOA_MV Crowdtruth PRF stats after 30 experiments:... ')
...
print('\nOA_MV JSD stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(jsds), np.max(jsds), np.min(jsds), np.std(jsds)))

print('\nOA_MV KL stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(kls), np.max(kls), np.min(kls), np.std(kls)))

print('\nOA_MV entropy similarity stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))

print('\nOA_MV entropy correlation stats after 30 experiments: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ents_correlation), np.max(ents_correlation), np.min(ents_correlation), np.std(ents_correlation)))

print('\nOA_MV crossentropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))

print('\n\nEVALUATING ON THE DEV SET I.E. THE FORNACIARA TEST SET')
print('OA_MV Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(dev_accs)*100, np.max(dev_accs)*100, np.min(dev_accs)*100, np.std(dev_accs)*100))
...
"""**PART TWO: PLANK ET AL STYLE WEIGHTING**

**Computing the Confusion Matrix using massimo style**
"""
...
class RNN_all(torch.nn.Module):
    ...
    def forward(self, wword_pad, wword_iis, cchar_pad, one_hot_labels, soft_labels, eval=False):
        word_sequence, word_context = self.word_encoder(wword_pad, wword_iis)
        word_attn= self.word_attention(word_sequence, 'word')
...
def get_predictions(model, eval_loader, mtl=False):
    ...
    for wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat in eval_loader:
        if not mtl:
            one_hot_pred, _,_,_ = model(wwordpad_bat, wwordiis_bat, ccharpad_bat, hhot_bat, ssoft_bat, True)
            one_hot_pred = one_hot_pred.detach().cpu().numpy()
            # the soft and hard predictions come from the same dense layer in the single task models.
            hard_preds.extend(np.argmax(one_hot_pred, 1))
            soft_preds.extend(one_hot_pred)
...
print('Plank_Gold Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(accs)*100, np.max(accs)*100, np.min(accs)*100, np.std(accs)*100))

print('\nPlank_Gold PRF stats after 30 epochs:... ')
avgs = ['averages'] + np.average(prfs, 0).tolist()
maxs = ['maximums'] + np.max(prfs, 0).tolist()
mins = ['minimums'] + np.min(prfs, 0).tolist()
stds = ['stds'] + np.std(prfs, 0).tolist()
...
print('\nPlank_Gold Crowdtruth PRF stats after 30 epochs:... ')
avgs = ['averages'] + np.average(ct_prfs, 0).tolist()
maxs = ['maximums'] + np.max(ct_prfs, 0).tolist()
mins = ['minimums'] + np.min(ct_prfs, 0).tolist()
stds = ['stds'] + np.std(ct_prfs, 0).tolist()
...
print('\nPlank_Gold JSD stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(jsds), np.max(jsds), np.min(jsds), np.std(jsds)))

print('\nPlank_Gold KL stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(kls), np.max(kls), np.min(kls), np.std(kls)))

print('\nPlank_Gold entropy similarity stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))

print('\nPlank_Gold entropy correlation stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ents_correlation), np.max(ents_correlation), np.min(ents_correlation), np.std(ents_correlation)))

print('\nPlank_Gold crossentropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
