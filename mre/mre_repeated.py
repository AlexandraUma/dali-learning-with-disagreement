...
Original file is located at
    https://colab.research.google.com/drive/1eCeRI1CHgaUBJnn3wDszajtaWaDhzuWO
...
for sent in data_dict:
    ...
    annotations = sent_info[-1]

    num_anns = sum(annotations)

    # repeated indices for getting train labels
    repeated_indices.append([item_idx for i in range(num_anns)])

    # repeated labels for class 0
    rep_labels = []
    for i in range(annotations[0]):
        rep_labels.append([1, 0])
    # repeated labels for class 1
    for i in range(annotations[1]):
        rep_labels.append([0, 1])
    # item's repeated label is ready, tensor loader will take care of shuffling
    repeated_train_labels.append(rep_labels)

    train_distr.append(annotations)
    distr = sent_info[-1]
    num_votes = sum(distr)
...
assert len(repeated_indices) == len(train_sents)
assert len(repeated_train_labels) == len(train_sents)

repeated_indices[0], repeated_indices[-1]

np.array(repeated_train_labels[0]), np.array(repeated_train_labels[-1])
...
class MyBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, labels=None, item_weights=None):
        ...
        auxiliary_loss = 0.0 # left over from full system

        cross_entropy = torch.mul(labels, softmax_scores.log())
        if item_weights:
            cross_entropy = torch.sum(cross_entropy, 1) * item_weights
        loss = -torch.sum(cross_entropy)

        outputs = (loss,) + (auxiliary_loss,) + outputs
        return outputs # (loss), logits, (hidden_states), (attentions)

"""**Repeated labelling mre**"""
num_repeats = [len(l) for l in repeated_train_labels]
num_repeats[:6]

repeated_input_ids = []
repeated_attention_masks = []
for in_id, attn_msk, num in zip(input_ids, attention_masks, num_repeats):
    repeated_input_ids.append([in_id.tolist() for i in range(num)])
    repeated_attention_masks.append([attn_msk.tolist() for i in range(num)])
...
for experiment in range(num_experiments):
    ...
    # shuffling and repeating the items for training the model
    ...
    shuffled_repeated_labels = [repeated_train_labels[i] for i in ids]
    shuffled_repeated_inputs = [repeated_input_ids[i] for i in ids]
    shuffled_repeated_masks = [repeated_attention_masks[i] for i in ids]

    # shuffling the items needed for evaluation
    ...
    for split_i in range(0, 5):
        ...
        # there's probably many better ways to do this
        train_input_ids, train_attn_masks, train_labels = [], [], []
        for i in range(975):
            if (i < start) or (i >= end):
                train_input_ids.extend(shuffled_repeated_inputs[i])
                train_attn_masks.extend(shuffled_repeated_masks[i])
                train_labels.extend(shuffled_repeated_labels[i])

        # train_input_ids = shuffled_repeated_inputs[:start] + shuffled_repeated_inputs[end:]
        # train_attn_masks = shuffled_repeated_masks[:start] + shuffled_repeated_masks[end:]
        # train_labels = shuffled_repeated_labels[:start] + shuffled_repeated_labels[end:]
...
print('Repeated Labelling Accuracy stats after 30 epochs: Avg %0.2f, Max %0.2f, Min %0.2f, Std %0.2f' %(np.average(accs)*100, np.max(accs)*100, np.min(accs)*100, np.std(accs)*100))

print('\nRepeated Labelling PRF stats after 30 epochs:... ')
...
print('\nRepeated Labelling Crowdtruth PRF stats after 30 epochs:... ')
...
print('\nRepeated Labelling JSD stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(jsds), np.max(jsds), np.min(jsds), np.std(jsds)))

print('\nRepeated Labelling KL stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(kls), np.max(kls), np.min(kls), np.std(kls)))

print('\nRepeated Labelling entropy similarity stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(similarity_ents), np.max(similarity_ents), np.min(similarity_ents), np.std(similarity_ents)))

print('\nRepeated Labelling entropy correlation stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ents_correlations), np.max(ents_correlations), np.min(ents_correlations), np.std(ents_correlations)))

print('\nRepeated cross entropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
