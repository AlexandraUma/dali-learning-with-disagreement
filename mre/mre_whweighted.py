...
whitehill = []
with open(DATAPATH+"/mre-inference.txt",encoding='utf8' ) as fd:
    rd = csv.reader(fd, delimiter=",")
    for row in rd:
        if row != []:
            whitehill.append(row)

whitehill_dict = {}
for item in whitehill[1:]:
    whitehill_dict[item[0]] = float(item[3])
...
for sent in data_dict:
    ...
    wh_weights.append(whitehill_dict[sent])
...
class MyBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, labels=None, item_weights=None):
        ...
        weighted_loss = torch.sum(cross_entropy, 1) * item_weights
...
for experiment in range(num_experiments):
    ...
    item_weights_ = (np.array(wh_weights)[ids]).tolist()
