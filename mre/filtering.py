# -*- coding: utf-8 -*-
"""mace_mre_biobert.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1WbZ_o7x2mjWorvIDjhHEWCiRV7FDc7-W
...
import json
with open(DATAPATH + '/mace_data/mapped_preds.json', 'r') as fp:
    mace_dict = json.load(fp)

"""**Training Data Preparation**"""

from scipy.special import softmax

# creating the train and development data. Actually mace not ds but I didn't change it
train_sents = []
train_experts = []
train_crowd = []
train_distr = []
...
for sent in data_dict:
    ...
    sent_ds = mace_dict[sent]
    mace_dummy = [0,0]
    mace_dummy[np.argmax(sent_ds)] = 1
    mace_labels.append(mace_dummy)
    mace_softs.append(sent_ds)


norm = [0.5, 0.5]
item_entropys = [entropy(scores)/entropy(norm) for scores in train_soft]
print(np.array(item_entropys))
...
datalen = len(labels)
avg_oa = np.average(item_oas)
right_indices = [i for i in range(len(item_oas)) if item_oas[i] >= avg_oa]
print(len(right_indices), avg_oa)
...
for experiment in range(num_experiments):
    ...
    # a different shuffle for each experimen
    ...
    for split_i in range(0, 5):
        ...
        train_indices = list(range(datalen))[:start] + list(range(datalen))[end:]
        train_indices = [idx for idx in train_indices if idx in right_indices]

        train_input_ids = np.array(input_ids_)[np.array(train_indices)]
        train_attn_masks = np.array(attention_masks_)[np.array(train_indices)]
        train_labels = np.array(labels_)[np.array(train_indices)]
