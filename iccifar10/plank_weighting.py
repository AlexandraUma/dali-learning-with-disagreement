...
''' Computing the Confusion Matrix using Massimo Style confusion '''
...
class ResNet(nn.Module):
    ...
    def forward(self, x, hard_labels=None, is_eval=True):
        out = self.conv1(x)
        out = self.norm1(out)
        out = self.relu1(out)
        ...
        if is_eval:
            ...
            softmax_scores = torch.nn.functional.softmax(out_1, 1)
            items_weights = get_confusion_weights(softmax_scores, hard_labels)
...
for i in range(NUM_EXPERIMENTS):
    ...
    plank_dictionary[str(i)] = test_pred
...
print('\nPlank_Gold entropy cross entropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
