...
def get_ct_labels(train_distr, n_classes=10):
    train_srs = []
...
train_ct = get_ct_labels(train_distr)
...
for i in range(NUM_EXPERIMENTS):
    ...
    trn_ds = TensorDataset(torch.tensor(train_images).float().to(device), torch.tensor(train_ct).float().to(device))
