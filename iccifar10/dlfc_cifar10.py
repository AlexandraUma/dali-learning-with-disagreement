...
dlfc_users = np.load(DATAPATH + 'cifar10H_anno_users.npy')
dlfc_status = np.load(DATAPATH + 'cifar10H_anno_status.npy')
...
train_users = dlfc_users[train_indices]
train_status = dlfc_status[train_indices]
...
class ResNet(nn.Module):
    ...
    def __init__(self, n=7, res_option='A', use_dropout=False):
        ...
        # lazy alexandra, hardcoding it like this. you know it is.
        num_users = 2553
        self.num_classes = 10
        beta = torch.eye(10)
    ...
    def forward(self, x, is_testing=True, anno_users=None, anno_status=None):
        out = self.conv1(x)
        out = self.norm1(out)
        out = self.relu1(out)
        ...
        if is_testing:
            ...
            judgements_one_hot = (torch.eye(hotsize)[valid_judgements]).to(device)     # [across_mention_annotations, num_classes]

            # extract the confusion matrices for the users who provided the annotations
            users_confusions = torch.index_select(self.global_user_confusion, 0, valid_users)  # [across_mention_annotations, num_classes, num_classes]
            ...
            duplicated_scores = out.unsqueeze(1)   # [num_mentions, 1, 4]
            duplicated_scores = duplicated_scores.repeat(1, max_anno_length, 1)    # [num_mentions, max_anno_length, num_classes]
            duplicated_scores = torch.reshape(duplicated_scores, (batch_size*max_anno_length, hotsize))  # [num_mentions * max_anno_length, num_classes]

            # corresponding mention scores for each annotation provided
            selected_scores = duplicated_scores[flattened_anno_status>-1]    # [across_mention_annotations, num_classes]
            # the expected annotator respone is a linear combination of the annotator confusion and the neural network prediction
            expected_user_responses = torch.matmul(users_confusions, selected_scores.unsqueeze(2)).squeeze(2)  # [across_mention_annotations, num_classes]

            ce = torch.mul(judgements_one_hot, nn.LogSoftmax(dim=1)(expected_user_responses))
            loss = -torch.sum(ce)
...
"""**TRAINING**"""
print('###################################Training Using MV Labels##########################################')

"""**Hard Labelling**"""

def train(loader_train, model, criterion, optimizer):
    model.train()
    for t, (X, y, a, b) in enumerate(loader_train):
        X_var = Variable(X)
        y_var = Variable(y)
        batch_users = Variable(a)
        batch_status = Variable(b)
        _, loss = model(X_var, False, batch_users, batch_status) 
        optimizer.zero_grad()
        loss.backward()
...
for i in range(NUM_EXPERIMENTS):
    ...
    trn_ds = TensorDataset(torch.tensor(train_images).float().to(device), torch.tensor(train_labels).long().to(device),
                           torch.tensor(train_users).long().to(device), torch.tensor(train_status).long().to(device))
...
with open(writepath+'cifar_dlc.jsonlines', 'w') as f:
