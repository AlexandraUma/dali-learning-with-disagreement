...
def get_ce_loss(scores, soft_labels):
    '''Args:
    ...
    cross_entropy = torch.mul(soft_labels, scores)
    loss = -torch.sum(cross_entropy)
    return loss


def get_kl_loss(scores, soft_labels):
    '''Args:
        true_labels: either one hot labels or a probability distribution
        predicted_labels: output of the model
    '''
    scores = torch.softmax(scores, 1) + 1e-12
    soft_labels = soft_labels + 1e-12
    loss = torch.sum(torch.mul(soft_labels, (torch.div(soft_labels, scores).log())))
    return loss


def get_inversekl_loss(scores, soft_labels):
    '''Args:
        true_labels: either one hot labels or a probability distribution
        predicted_labels: output of the model
    '''
    scores = torch.softmax(scores, 1) + 1e-12
    soft_labels = soft_labels + 1e-12
    loss = torch.sum(torch.mul(scores, (torch.div(scores, soft_labels).log())))
    return loss


def get_mse_loss(scores, soft_labels):
    '''Args:
        true_labels: either one hot labels or a probability distribution
        predicted_labels: output of the model
    '''
    scores = torch.softmax(scores, 1) + 1e-4
    soft_labels = soft_labels + 1e-4
    loss = torch.nn.MSELoss(reduction='sum')(soft_labels, scores)
    return loss
...
def train(loader_train, model, criterion, optimizer):
    ...
    for t, (X, y) in enumerate(loader_train):
        ...
        loss = get_inversekl_loss(scores, y)
        #loss = get_mse_loss(scores, y)
...
for i in range(NUM_EXPERIMENTS):
    ...
    trn_ds = TensorDataset(torch.tensor(train_images).float().to(device), torch.tensor(train_stdnorm_soft).float().to(device))
    ...
    for num_epochs in SCHEDULE_EPOCHS:
        ...
        for epoch in range(num_epochs):
            ...
            if dev_f1 > best_val_f1:
                ...
                torch.save(model.state_dict(), DATAPATH+'best_invklsoftmodel.pt')

        learning_rate *= 0.1

    model = ResNet(res_option='A').to(device)
    model.load_state_dict(torch.load(DATAPATH+'best_invklsoftmodel.pt'))
