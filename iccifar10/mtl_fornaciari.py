...
class ResNet(nn.Module):
    ...
    def forward(self, x, hard_labels=None, soft_labels=None, is_eval=True):
        out = self.conv1(x)
        out = self.norm1(out)
        out = self.relu1(out)
        ...
        if is_eval:
            ...
            soft_loss = get_inversekl_loss(out_2, soft_labels)
...
def predict_using_model(model, dataloader):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            preds_hard, preds_soft  = model(x)
            preds_hard = torch.softmax(preds_hard, 1).detach().cpu().numpy()
            pred_labels.extend(np.argmax(preds_hard, 1).tolist())

            preds_soft = torch.softmax(preds_soft, 1).detach().cpu().numpy()
            pred_probs.extend(preds_soft.tolist())
...
for i in range(NUM_EXPERIMENTS):
    ...
    trn_ds = TensorDataset(torch.tensor(train_images).float().to(device), torch.tensor(train_labels).long().to(device),
                           torch.tensor(train_stdnorm_soft).float().to(device))
    ...
    mtlHard_dictionary[str(i)] = predicted_test_labels
    mtlSoft_dictionary[str(i)] = test_pred
...
with open(writepath+'cifar_hard_' + name + '.jsonlines', 'w') as f:
    json.dump(mtlHard_dictionary, f)

with open(writepath+'cifar_soft_' + name + '.jsonlines', 'w') as f:
