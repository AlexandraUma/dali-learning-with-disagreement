...
"""**The Train Set**"""
train_counts = crowd_counts[train_indices]
print(train_counts[0])
sheng_indices = []
sheng_multipliers = []
sheng_labels = []
for idx in range(len(train_indices)):
    item_counts = train_counts[idx]
    for i in range(NUM_CLASSES):
        if item_counts[i] != 0:
            sheng_indices.append(idx)
            sheng_multipliers.append(item_counts[i])
            sheng_labels.append(i)

print(len(sheng_indices), len(train_indices))
print(sheng_multipliers[:2])
...
sheng_train_images = train_images[sheng_indices]
print(sheng_train_images.shape)

sheng_train_labels = np.array(sheng_labels)
print(sheng_train_labels.shape, sheng_train_labels[:3])
...
"""**TRAINING**"""
print('###################################Training Using Sheng Labels##########################################')

"""**Hard Labelling**"""

def get_ce_loss(scores, labels_long, weights):
    '''Args:
        true_labels: either one hot labels or a probability distribution
        predicted_labels: output of the model
    '''
    true_labels = torch.nn.functional.one_hot(labels_long, NUM_CLASSES)
    scores = nn.LogSoftmax(dim=1)(scores)
    cross_entropy = torch.mul(true_labels.float(), scores)
    # weighting the items using the sheng multipliers
    cross_entropy = torch.sum(cross_entropy, 1) * weights
...
def train(loader_train, model, criterion, optimizer):
    model.train()
    for t, (X, y, w) in enumerate(loader_train):
        X_var = Variable(X)
        y_var = Variable(y)
        weights = Variable(w)
        scores = model(X_var) 
        ...
        loss = get_ce_loss(scores, y, weights)
...
for i in range(NUM_EXPERIMENTS):
    ...
    trn_ds = TensorDataset(torch.tensor(sheng_train_images).float().to(device), torch.tensor(sheng_train_labels).long().to(device),
                           torch.tensor(sheng_multipliers).float().to(device))
    ...
    for num_epochs in SCHEDULE_EPOCHS:
        ...
        for epoch in range(num_epochs):
            ...
            if dev_f1 > best_val_f1:
                ...
                torch.save(model.state_dict(), DATAPATH+'best_shengmodel.pt')

        learning_rate *= 0.1

    model = ResNet(res_option='A').to(device)
    model.load_state_dict(torch.load(DATAPATH+'best_shengmodel.pt'))

    model.eval()
    _, test_pred, predicted_test_labels = predict_using_model(model, test_dl)

    sheng_dictionary[str(i)] = test_pred
...
print('\nSheng CE cross entropy stats after 30 epochs: Avg %0.4f, Max %0.4f, Min %0.4f, Std %0.4f' %(np.average(ce_results), np.max(ce_results), np.min(ce_results), np.std(ce_results)))
