...
class Trainer(object):
    ...
    def tensorize_document(self, example, is_training, gold_eval=False):
        ...
        if not is_training:
           ...
           targets = np.argmax(np.array(example['mace_posterior']), 1)   # for plank
        ...
        external_weights = np.array([observed_agreement(a) for a in mv_distrs])
        #external_weights = np.array(example['wh_conf'])  
    ...
    def create_dataset(self, e, plank=False):
        ...
        mention_labels, features = torch.from_numpy(e[10]).float().to(device), torch.from_numpy(e[11]).float().to(device)
        external_weights = torch.from_numpy(e[12]).float().to(device)
    ...
    def run_experiment(self, exp, smythe, train_documents, dev_documents, test_documents, num_epochs, num_experiments=10):
        ...
        for exp_idx in range(num_experiments):
            ...
            for epoch in range(num_epochs):
                ...
                for tensorized_document in train_documents:
                    ...
                    _, _, l1, l2 = model(context_word_emb, head_word_emb, lm_emb, char_index, text_len, gold_starts, gold_ends, features, one_hot_labels, external_weights, mv_distrs, is_training)
        ...
        if exp == 'stl':
            if not smythe:
                exp_name = 'OA_Weighted' # OA or WH_Weighted
            else:
                exp_name = 'PlankWeightedGold'
       
        name = 'plankWeightedGold'
