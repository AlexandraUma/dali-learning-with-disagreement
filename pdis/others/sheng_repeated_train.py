...
def get_repeated_indices_multipliers(mv_distrs):
    sheng_labels = []
    item_multiplier = []
    repeated_indices = []
    num_classes = mv_distrs.shape[1]
    num_mentions = mv_distrs.shape[0]
    for midx in range(num_mentions):
        a = mv_distrs[midx]
        for lab_idx in range(num_classes):
            ...
            if lab_count > 0:
                dummy[lab_idx] = 1
                sheng_labels.append(dummy)
                item_multiplier.append(lab_count)
                repeated_indices.append(midx)
    return np.array(repeated_indices), np.array(sheng_labels), np.array(item_multiplier)
...
def get_not_filtered_indices(mv_distrs):
    remaining_indices = []
    aggrs = [observed_agreement(a) for a in mv_distrs]
    avg_aggr = np.average(aggrs)
    for midx in range(len(mv_distrs)):
        if aggrs[midx] > avg_aggr:
            remaining_indices.append(midx)
    return np.array(remaining_indices)
...
class Trainer(object):
    ...
    def tensorize_document(self, example, is_training, gold_eval=False):
        ...
        if not is_training:
            ...
            external_weights = np.ones_like(targets)
        else:
            #repeated_indices, sheng_labels, item_multiplier = get_repeated_indices_multipliers(mv_distrs)
            #targets = np.argmax(sheng_labels, 1)
            #gold_starts = gold_starts[repeated_indices]
            #gold_ends = gold_ends[repeated_indices]
            #features = features[repeated_indices]
            #external_weights = item_multiplier
            # for filtering
            remaining_indices = get_not_filtered_indices(mv_distrs)
            ##targets = np.argmax(mv_distrs, 1)[remaining_indices]
            targets = np.argmax(example['mv_labels'], 1)[remaining_indices]
            gold_starts = gold_starts[remaining_indices]
            gold_ends = gold_ends[remaining_indices]
    ...
    def run_experiment(self, exp, smythe, train_documents, dev_documents, test_documents, num_epochs, num_experiments=10):
        ...
        for exp_idx in range(num_experiments):
            ...
            for epoch in range(num_epochs):
                ...
                for tensorized_document in train_documents:
                    ...
                    _, _, l1, l2 = model(context_word_emb, head_word_emb, lm_emb, char_index, text_len, gold_starts, gold_ends, features, one_hot_labels, soft_labels, external_weights, is_training)
            ...
            #trueHard_dictionary[str(exp_idx)] = [item.tolist() for item in test_hard]
            #trueSoft_dictionary[str(exp_idx)] = [item.tolist() for item in test_softs]
            distrs_dictionary[str(exp_idx)] = [item.tolist() for item in test_distrs]
        ...
        if exp == 'stl':
            if not smythe:
                exp_name = 'SHENG' # 'SHENG' #'Integrated'
            else:
        ...
        #with open(writepath+'pdis_trueLabels.jsonlines', 'w') as f:
        #    json.dump(trueHard_dictionary, f)

        #with open(writepath+'pdis_trueSofts.jsonlines', 'w') as f:
        #    json.dump(trueSoft_dictionary, f)

        with open(writepath+'pdis_distrs.jsonlines', 'w') as f:
            json.dump(distrs_dictionary, f)
    ...
    def get_predictions(self, model, eval_documents, mtl=False):
        ...
        for tensorized_document in eval_documents:
            ...
            if mtl:
                one_hot_pred, soft_pred, _, _ = model(context_word_emb, head_word_emb, lm_emb, char_index, text_len, gold_starts, gold_ends, features, one_hot_labels, soft_labels, external_weights, is_training)
                ...
                one_hot_pred, _, _, _ = model(context_word_emb, head_word_emb, lm_emb, char_index, text_len, gold_starts, gold_ends, features, one_hot_labels, soft_labels, external_weights,is_training)
                hard_preds.extend(one_hot_pred.argmax(-1).detach().cpu().numpy())
