...
class ISModel(nn.Module):
    ...
    def get_plank_matrix(self, distrs):
        matrices = [[0 for i in range(self.num_classes)] for i in range(self.num_classes)]
        sum_pairs = 0
        for inst in distrs.cpu().numpy():
            a = []
            for lab in range(len(inst)):
                lab_count = int(inst[lab])
                a.extend([lab]*lab_count)
            all_pairs = list(combinations(a, 2))
            sum_pairs += len(all_pairs)
            for i in range(self.num_classes):
                for j in range(self.num_classes):
                    matrices[i][j] += all_pairs.count((i, j))
        plank_matrix = np.array([[0.0 for i in range(self.num_classes)] for i in range(self.num_classes)])
        for i in range(self.num_classes):
            for j in range(i, self.num_classes):
                if i != j:
                    i_j_confusion = (matrices[i][j] + matrices[j][i]) / sum_pairs
                    plank_matrix[i][j] = i_j_confusion
                    plank_matrix[j][i] = i_j_confusion
                else:
                    plank_matrix[i][i] = 0.0 # matrices[i][i] / sum_pairs
        return plank_matrix


    def get_plank_weights(self, predicted_scores, true_one_hot, mv_distrs):
        plank_matrix = self.get_plank_matrix(mv_distrs)
        #print(plank_matrix)
        predictions = np.argmax(predicted_scores.detach().cpu().numpy(), 1)
        true_labels = torch.argmax(true_one_hot, 1).cpu().numpy()
        system_majority = np.stack([predictions, true_labels], 1)
        weights = 1 - to_cuda(torch.tensor([plank_matrix[i][j] for i,j in system_majority]))
        #print(weights)
        return weights
    ...
    def forward(self, context_word_emb, head_word_emb, lm_emb, char_index, text_lens, gold_starts, gold_ends, features, one_hot_labels, item_weights, mv_distrs, is_training):
        ...
        if is_training:
            ...
            if self.smythe=='plank':  # for plank style, item_weights = None
                #print('using plank weights')
                item_weights = self.get_plank_weights(softmax_scores, one_hot_labels, mv_distrs)
            #print(item_weights)
            loss = -torch.sum(cross_entropy, 1) * item_weights
            return softmax_scores, None, loss.sum(), 0.0
        else:
