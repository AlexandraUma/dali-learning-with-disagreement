...
class ISModel(nn.Module):
    def __init__(self, exp='stl', smythe=None):
        ...
        self.smythe = smythe
        if self.exp == 'mtl':
            self.soft_scorer = DenseLayer(span_emb_dim, self.hidden_layer_size, self.dropout, self.num_classes)


    def forward(self, context_word_emb, head_word_emb, lm_emb, char_index, text_lens, gold_starts, gold_ends, features, one_hot_labels, soft_labels, is_training):
        ...
        softmax_scores = F.softmax(information_status_scores, 1) + EPSILON # YOU GET NAN LOSSES OTHERWISE
        if self.exp != 'mtl':
            if self.smythe == 'ce':
            ...
            elif self.smythe == 'inverse_kl':
                loss = torch.sum(torch.mul(softmax_scores, (torch.div(softmax_scores, soft_labels).log())))
            ...
            # soft loss
            soft_pred = self.soft_scorer(span_emb) 
            softmaxed = F.softmax(soft_pred, 1) + EPSILON
            #softmaxed = torch.clamp(softmaxed, min=0.0, max=1.0)
            soft_loss = torch.sum(torch.mul(softmaxed, (torch.div(softmaxed, soft_labels).log())))
